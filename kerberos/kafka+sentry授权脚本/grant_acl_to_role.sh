...
then
	...
	do 
		echo "分配权限$acl"
		sudo -u kafka kafka-sentry  -gpr -r $role_name -p "Topic=$topic->action=$acl"
	done
	
	echo "将$role_name角色添加到$input_name组中"
	sudo -u kafka kafka-sentry -arg -r $role_name -g $input_name
